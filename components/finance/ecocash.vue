<template>
   ...
   <div class="mt-4">
   <v-btn depressed  large color="info" @click="vdialog=true" block>ECOCASH  PAYMENT</v-btn>

   <v-dialog v-model="vdialog" width="400px">
    <v-form ref="form" lazy-validation>
        <v-card>
        <v-card-title>ECOCASH PAYMENTS</v-card-title>
        <v-card-text>
   ...
   <v-dialog v-model="confirmDialog" width="400px">
      ...
      type="info"
    >
         Please check mobile and enter your ECOCASH pin to authorize payment
...
export default {
        ...
        this.$swal({
          ...
          if (result.isConfirmed) {
            try {
              ...
              await this.$axios
                .post("/client-receipting/mobilepayment", this.form)
                .then((res) => {
 ...
 async checkpayment(){
   ...
   try {
              ...
              await this.$axios
                .get("/client-receipting/mobilepayment/"+this.recordId)
                .then(async(res) => {
                    this.$swal(res.data.status,res.data.message,res.data.status)
                     await this.$store.dispatch('invoice/getById',this.id)
